    ...
    try {
        ...
        if (isSuperAdminRegistered.rowCount > 0) {
            ...
            if (isAlreadyRegistered.rowCount == 0) {
                ...
                let s3 = dbScript(db_sql['Q2'], { var1: mysql_real_escape_string(name), var2: mysql_real_escape_string(email.toLowerCase()), var3: encryptedPassword, var4: mysql_real_escape_string(avatar) })
...
module.exports.changePassword = async (req, res) => {
    try {
        ...
        if (oldPassword === newPassword) {
            return handleResponse(res, 409, false, "New Password cannot be the same as Old Password");
        }
        ...
        let findSuperAdmin = await connection.query(s1)
        if (findSuperAdmin.rowCount > 0) {
            let matchOldPassword = await bcrypt.compare(oldPassword, findSuperAdmin.rows[0].password)
            if (matchOldPassword) {
                const encryptedPassword = await bcrypt.hash(newPassword, 10);
                let _dt = new Date().toISOString()
                let s2 = dbScript(db_sql['Q4'], { var1: encryptedPassword, var2: _dt, var3: id })
    ...
    try {
        ...
        if (findSuperAdmin.rowCount > 0) {
            ...
            if (uploadAvatar.rowCount > 0) {
                await connection.query("COMMIT")
                return handleResponse(res, 201, true, "Profile Pic uploaded successfully.", path);
            } else {
                await connection.query("ROLLBACK")
                return handleSWRError(res);
            }
...
module.exports.createCompany = async (req, res) => {
    try {
        let { id } = req.user
        let { company_name, company_email, company_contact_number, max_cards, contact_person_name, contact_person_email } = req.body
        if (!company_name || !company_email || !company_contact_number || !max_cards || !contact_person_name || !contact_person_email) {
            return handleResponse(res, 400, false, "Please provide all the Fields.")
        }
        ...
        let findSuperAdmin = await connection.query(s1)
        if (findSuperAdmin.rowCount > 0) {
            let s2 = dbScript(db_sql['Q5'], { var1: mysql_real_escape_string(company_name), var2: mysql_real_escape_string(company_email) })
    ...
    try {
        ...
        if (findSuperAdmin.rowCount > 0) {
            ...
            if (getCompanyDetails.rowCount > 0) {
                return handleResponse(res, 200, true, "Company Details", getCompanyDetails.rows)
            } else {
            ...
            if (findCompany.rowCount > 0) {
                ...
                if (findCompany.rows[0].status !== 'deactivated') {
                    ...
                    if (isAdminAlreadyReg.rowCount == 0) {
                        ...
                        let s4 = dbScript(db_sql['Q9'], { var1: mysql_real_escape_string(first_name), var2: mysql_real_escape_string(last_name), var3: mysql_real_escape_string(email.toLowerCase()), var4: encryptedPassword, var5: mobile_number, var6: company_id, var7: findSuperAdmin.rows[0].id, var8: "Admin", var9: avatar, var10: findCompany.rows[0].company_name })
                    ...
                    return handleResponse(res, 400, false, "Company Account is Deactivated")
                }
            } else {
...
module.exports.editCompanyDetails = async (req, res) => {
    try {
        let { id } = req.user
        let {
            company_id, company_name, company_email, company_contact_number, max_cards, contact_person_name, contact_person_email,
        ...
        if (findSuperAdmin.rowCount > 0) {
            ...
            if (findCompany.rowCount > 0) {
                if (findCompany.rows[0].used_cards > max_cards) {
                    return handleResponse(res, 400, false, "Maximum number of cards can not be less than used cards")
                }
                let s3 = dbScript(db_sql['Q12'], { var1: mysql_real_escape_string(company_name), var2: company_email.toLowerCase(), var3: company_contact_number, var4: max_cards, var5: mysql_real_escape_string(contact_person_name), var6: contact_person_email.toLowerCase(), var7: description, var8: company_address, var9: company_logo, var10: company_website, var11: contact_person_designation, var12: contact_person_mobile, var13: latitude, var14: longitude, var15: company_id })
                let editCompanyData = await connection.query(s3)

                if (editCompanyData.rowCount > 0) {
                    await connection.query("COMMIT")
                    return handleResponse(res, 200, true, "Company details Updated successfully")
                } else {
                    await connection.query("ROLLBACK")
                    return handleSWRError(res);
                }
