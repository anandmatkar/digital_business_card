...
const superAdminValidation = {
  ...
  forgetPasswordSAValidation: async (req, res) => {
    const validationRules = [
      body("email")
        .trim()
        .isLength({ min: 1 })
  ...
  resetPasswordSAValidation: async (req, res) => {
    const validationRules = [
      body("password")
        .trim()
        .isLength({ min: 1 })
  ...
  createCompanyValidation: async (req, res) => {
    const validationRules = [
      ...
      body("max_cards").isNumeric().withMessage("Max Cards must be numeric"),
  ...
  editCompanyDetailsValidation: async (req, res) => {
    const validationRules = [
      ...
      body("contact_person_email")
        .isEmail()
        .withMessage("Invalid Company Person Email address"),
      body("max_cards")
        .isNumeric()
        .withMessage("Provide Valid Card Number (In Digits)"),
      ...
      body("company_address")
        ...
        .withMessage(
          "Company Address must not contain only whitespace characters"
        ),
      body("company_logo").optional().trim(),
      body("company_website").optional().trim(),
      body("contact_person_designation").optional().trim(),
      body("contact_person_mobile").optional().trim(),
    ];
